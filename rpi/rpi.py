...
def next_power_of_two(x: int):
    ...
    next_power_of_two(768) returns (1, 1)  --> 1024 is next power of 2 for the
    input x=768. This can be devided once by 1024 with a resulting quotient of 1,
    hence the return tuple of (1, 1)

    next_power_of_two(468349) returns (512, 1) --> 524288 is next power of 2 for
    the input x=468349. Devided by 1024 gives a quotient of 512, hence the return
    tuple of (512, 1)

    next_power_of_two(114638784) returns (128, 2) --> 134217728 is next power of 2
    for the input x=114638784. This can be devided twice by 1024 with a quotient of
    128, hence the return tuple of (128, 2)
    """
    if x == 0:
        res = 1
    else:
...
def get_command_location(arg: str):
    """
    Return the location of the command 'arg' as a string. If 'arg' is not found,
    return empty string.
    """
...
def get_device_cpu_info():
    ...
    """
    cpu_info = {}
    cmd_string1 = "{} | {} -i 'architecture|core\(s\)|vendor|model|min|max'".format(
        lscpu, egrep
    )
    cmd_string2 = (
        cat
    ...
    out = subprocess.Popen(
        cmd_string1, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    stdout, _ = out.communicate()
    ...
    cpu_info["Clock Speed [MHz] (min|max)"] = "{} | {}".format(
        cpu_clock_speed_min, cpu_clock_speed_max
    )

    # get serial number
    out = subprocess.Popen(
        cmd_string2, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    stdout, _ = out.communicate()
...
def get_device_memory_installed():
    ...
    The first value is the memory size. The second value is an index for the units to be
    applied (0 = 'kB', 1 = 'MB', 2 = 'GB', 3 = 'TB').
    ...
    """
    cmd_string = (
        cat + " /proc/meminfo | " + egrep + " -i 'memtotal' | " + awk + " '{print $2}'"
    )
    out = subprocess.Popen(
        cmd_string, shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
...
def get_network_interfaces():
    ...
    interface its IP and MAC address (if allocated), where the string returns the MAC
        address of the first physical	interface in lower characters.
    """
    mac_address = ""
    cmd_string = (
        ipaddr
